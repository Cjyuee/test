...
from trainer import  Qwen2VLGRPOVLLMTrainer,Qwen2VLGRPOTrainerSAT
...
def main(script_args, training_args, model_args):
    ...
    # replace your own data path
    # dataset_prefix = "/mnt/hwfile/gveval/liming/new_data/"
    # dataset_prefix = "/storage/sealgo/Multimodal/dongshiqi/caojiayue/project/VisualThinker-R1-Zero/src/data/SAT/"
    dataset_prefix = "/root/autodl-tmp/datasets/MathV360K/"
    dataset_path = "mathv360k_train.json"

    import json
    # load json file
    with open(dataset_prefix + dataset_path, 'r') as f:
        sat_dataset = json.load(f)
    # 逐样本构造问题模板（应该根据不同的数据集修改不同的构造问题模板）
    ...
    trainer_cls = Qwen2VLGRPOTrainerSAT if not training_args.use_vllm else Qwen2VLGRPOVLLMTrainer
    print("using: ", trainer_cls)

    # Initialize the GRPO trainer
    trainer = trainer_cls(
        model=model_args.model_name_or_path, # 输入参数中模型的位置
        reward_funcs=reward_funcs, # 奖励函数
        args=training_args,
        train_dataset=dataset[script_args.dataset_train_split], # 训练集
        eval_dataset=dataset[script_args.dataset_test_split] if training_args.eval_strategy != "no" else None, # 验证集
        peft_config=get_peft_config(model_args), # 微调参数设置
        attn_implementation=model_args.attn_implementation,
        max_pixels=script_args.max_pixels,
        min_pixels=script_args.min_pixels,
    ) # 这里初始化完成之后会跳转到grpo_trainer_sat.py里的Qwen2VLGRPOTrainerSAT类内,改模型型号在那里面改
