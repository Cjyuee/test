...
# # datasets 和 hub 是否离线模式，0 表示联网
# os.environ["HF_DATASETS_OFFLINE"] = "0"  
# os.environ["HF_HUB_OFFLINE"] = "0"      

# # 设置国内镜像 URL
# # 1. 数据集下载镜像（可以换成清华/科大镜像）
# os.environ["HF_DATASETS_DOWNLOAD_URL"] = "https://mirrors.tuna.tsinghua.edu.cn/hugging-face-datasets/"

# # 2. 模型/Hub 下载镜像（可以换成 ghproxy 或清华镜像）
# os.environ["HF_ENDPOINT"] = "https://ghproxy.com/https://huggingface.co"

# # 可选：pip 使用镜像安装 transformers/datasets
...
if __name__ == "__main__":
    ...
    # 如果是下载数据集到本地的话用下面这一行，把路径换成存储数据集的路径即可
    # cv_bench = load_dataset("/storage/sealgo/Multimodal/dongshiqi/caojiayue/project/datasets/CVBench", split="test")
    ...
    if not PRECOMPUTED_RESULT:
        ...
        resp_messages = []
        print(f'prompt{args.use_reasoning_prompt}')
        for i, example in tqdm(enumerate(cv_bench)):
            if args.use_reasoning_prompt:
                if 'direct' in args.model_path.lower():
    ...
    for i, (input_example, model_resp_output) in enumerate(zip(cv_bench, all_resp_outputs)):
        ...
        parsed_response = parse(short_response, extraction_config=[LatexExtractionConfig(), ExprExtractionConfig(),
                                                                   StringExtractionConfig(strings=("A", "B", "C", "D",
                                                                                                   ...
                                                                                                   '(D)', '(E)',
                                                                                                   '(F)') + tuple(
                                                                       input_example['choices']))])
        parsed_answer = [example_answer.lower(), example_answer, ground_truth[1], ground_truth[1].lower(),
                         ground_truth.lower(), ground_truth]
